...
public record EndAfter(IntervalVar interval, IntExpression value) implements SymbolicBoolExpression {
    ...
    public IntExpression mapSubexpressions(Function<Expression, Expression> f) {
        return new EndAfter((IntervalVar) f.apply(interval), (IntExpression) f.apply(value));
    }
    ...
    private boolean isTriviallyTrue() {
        return interval.isPresent() && interval.endMin() >= value.max();
    }

    private boolean isTriviallyFalse() {
        return interval.isPresent() && interval.endMax() < value.min();
    }
