 ...
 */
public record CPMinusCumulFunction(CPCumulFunction left, CPCumulFunction right) implements CPCumulFunction {
    ...
    public CPIntVar heightAtStart(IntervalVar interval) {
        if (!inScope(interval)) {
            ...
            if (!left.inScope(interval)) {
                return CPFactory.minus(right.heightAtStart(interval));
                ...
                return CPFactory.sum(leftHeight, CPFactory.minus(rightHeight));
            }
        }
    }
    ...
    public CPIntVar heightAtEnd(IntervalVar interval) {
        if (!inScope(interval)) {
            ...
            if (!left.inScope(interval)) {
                return CPFactory.minus(right.heightAtEnd(interval));
            } else if (!right.inScope(interval)) {
